...
class Verification:
    ...
    def verify_chain(cls, blockchain):
        ...
        for (index, block) in enumerate(blockchain):
            ...
            if not cls.valid_proof(
                # block.transactions[:-1], block.previous_hash, block.proof # marked because no reward is given
                block.transactions[:], block.previous_hash, block.proof
            ):
    ...
    @staticmethod
    def verify_transaction(transaction, get_product_stock, check_funds=True):
    ...
    @classmethod
    def verify_transactions(cls, open_transactions, get_product_stock):
        """Verifies all open transactions."""
        return all(
            [
                cls.verify_transaction(tx, get_product_stock, False)
